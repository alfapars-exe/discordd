...
type ChannelMuteDurationPickerProps = {
  channelId: string;
  x: number;
  y: number;
  onClose: () => void;
...
function ChannelMuteDurationPicker({ channelId, x, y, onClose }: ChannelMuteDurationPickerProps) {
  const { t } = useTranslation("channels");
  const muteChannel = useChannelStore((s) => s.muteChannel);
  ...
  async function handleSelect(duration: string) {
    const ok = await muteChannel(channelId, duration);
    if (ok) {
      addToast("success", t("channelMuted"));
    }
    onClose();
  }
...
export default ChannelMuteDurationPicker;
